//------------------------------------------------------------------------------
//-- modified-konduktiva-revised.js
...
function lsystem(inputString, rules, generations) {
    if (generations > 0) {
        return lsystem(parseString(inputString, rules), rules, generations - 1)
    } else {
        return inputString
    }
}
...
function randomCharacterMapFromArray(inputArray) {
    ...
    if (allChars.length < inputArray.length) {
        return "error: inputArray is longer than array of alphabetic characters"
    };
    let outputMap = {};
    charactersToUse = takeN(allChars, inputArray.length);
    let shuffled = shuffle(inputArray);
    charactersToUse.forEach((x, i) => {
        outputMap[x] = shuffled[i]
    })
    return outputMap
}
...
function randomMap(keyArray, valueArray) {
    if (keyArray.length < valueArray.length) {
        return "error: key array is longer than value array"
    };
    let outputMap = {};
...
function getMinIndex(inputArray) {
    let currentMin = [];
    for (let i = 0; i < inputArray.length; i++) {
        if (currentMin[0] == undefined) {
            currentMin[0] = [0, inputArray[i]]
        } else if (inputArray[i] < currentMin[0][1]) {
            currentMin = [
                [i, inputArray[i]]
            ]
        } else if (inputArray[i] == currentMin[0][1]) {
            currentMin.push([i, inputArray[i]])
        }
    }
    return currentMin
}
...
function getMaxIndex(inputArray) {
    let currentMax = [];
    for (let i = 0; i < inputArray.length; i++) {
        if (currentMax[0] == undefined) {
            currentMax[0] = [0, inputArray[i]]
        } else if (inputArray[i] > currentMax[0][1]) {
            currentMax = [
                [i, inputArray[i]]
            ]
        } else if (inputArray[i] == currentMax[0][1]) {
            currentMax.push([i, inputArray[i]])
        }
    }
    return currentMax
}
...
function increaseDensity(minVal, ratio, inputArray) {
    ...
    if (max[0][1] <= minVal) {
        console.log("max is already at minVal");
        return inputArray
    } else {
...
//one of the arrays is the sizes of circle the other is time.
    ...
    nearestLookup(time) {
        ...
        if (lookupTime >= this.keys[this.keys.length - 1]) {
            ...
            if ((lookupTime - lower) < (higher - lookupTime)) {
                ...
                output = this.values[(filteredTime.length)]
            }
        }
        return output
    }
...
function calculateDensity(env, playerName, time) {
    ...
    let densityFuncMaps = dGraphs.map((x) => {
        if (Object.keys(x).includes(playerName)) {
            return x[playerName]
        } else if (Object.keys(x).includes('default')) {
            return x.default
        }
        // this has to be a QuantizedMap
        // else {return x => 1}
        else {
            return linearFunctionQuantizedMap([new Point(0, 1), new Point(1, 1)])
        }
    });
...
function generateAndAddRhythms(env, n, baseName, onsetValues, minIOI, ratios, seedLengths, noOfSeeds, phraseLength) {
    ...
    names.forEach((name, i) => {
        return env.rhythmMaps[name] = rhythmMaps[i]
    })
}
...
function mask(player, maskMap, beat, probability) {
    ...
    if (maskVal == true && (Math.random() < probability)) {
        maskVal = true
    } else {
        maskVal = false
    };
...
class MusicalEnvironment {
    ...
    scheduleEvents(player) {
        ...
        if (onsetsAfterLastScheduled[0] !== undefined) {
            ...
            if (player.verbose == true) {
                console.log("these are the times for events of player " + player + ": " + times)
            };
        ...
        if (player.verbose == true) {
            console.log("last scheduled time: " + this.lastScheduledTime)
        }
    }
    ...
    togglePlayer(p) {
        if (this.players[p].status == 'playing') {
            this.stop(p)
        } else {
            this.play(p)
        }
...
function simpleSamplePattern(env, playerName, sampleName, sampleIndex) {
    addSamplePattern(e, playerName, new QuantizedMap(4, [0], [{
        name: sampleName,
        index: sampleIndex
    }]));
    return playerName
...
class RhythmPattern {
    ...
    add(env) {
        ...
        env.players[this.voice].rhythmMap = this.patternName;
        let mask = flipBooleans(this.bools);
        env.players[this.voice].maskMap = this.patternName;
        if (env.maskMaps[this.patternName] != undefined) {
            console.log("Overwriting rhythm pattern")
        }
...
//STEVE Mod 2:
//Modification of the RhythmPattern class:
//Change to capitized
class animationPattern {
    constructor(createdPlayer, n, l, i, b) {
        this.voice = createdPlayer;
        this.patternName = n;
        this.patternLength = l;
        this.IOIs = i;
        this.bools = b;
